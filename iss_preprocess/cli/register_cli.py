...
@click.option(
    "-n",
    "--prefix",
    help="Path prefix, e.g. 'genes_round_1_1'. If None use reference acquisition",
)
@click.option(
    "--roi",
    "-r",
    multiple=True,
    type=int,
    help="ROI(s) to process. If not provided will use all ROIs",
)
def register_adjacent_tiles(path, prefix=None, roi=None):
    """Register adjacent tiles within a single round."""
    from iss_preprocess.pipeline.stitch import register_all_rois_within

    register_all_rois_within(path, prefix, roi2use=roi)

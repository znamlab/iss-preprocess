...
def call_genes(bases, codebook):
    """
    Assigned provided sequences to genes based on the provided codebook.

    Args:
        bases (numpy.ndarray): ROIs x rounds array of base IDs generated by
            `basecall_rois`.
    ...
    errors = []
    for s in bases:
        seq = iss.call.BASES[s]
        dist_series = codebook['seq'].apply(lambda s: hamming(list(s), seq)*len(s))
